(function (window)
{
	...
	function AudioBuffer(numberOfChannels, length, sampleRate)
	{
		...
		for (var i = 0; i < numberOfChannels; i++)
			channelBuffers[i] = new Float32Array(length);

		this.getChannelData = function (channel) { return channelBuffers[channel]; };

		this.__copyData = function (channel, sourceOffset, target, offset, count)
		{
			var data = channelBuffers[channel];

			for (var i = 0; i < count; i++)
			{
				target[i + offset] = data[i + sourceOffset];
			}
		};
	}
	/*jslint bitwise: true */
	...
	function AudioRequest(url, async)
	{
		...
		this.send = function ()
		{
			...
			loadAif(function (data)
			{
				if (data)
				{
					var buffer = request.buffer = new AudioBuffer(2, data.length, 44100);
	...
	function AudioBufferSourceNode(context)
	{
		...
		function pullDataFromTheBuffer(data, time, via)
		{
			...
			if (isOn && buffer)
			{
				var tail = buffer.length - currentOffset;
	...
	{
		ConvolverNode.call(this, context, 1, 1);
	}
	...
	function AudioDestinationNode(context, tickCallback)
	{
		...
		function tick()
		{
			...
			if (buffer)
			{
				...
				{
					buffer = buffer.subarray(written, buffer.length - written);
					return;
				}
				else
				{
			...
			if (currentOffset + PREBUFFER_SIZE >= writeOffset)
			{
				...
				{
					buffer = data.subarray(written, data.length - written);
				}
	...
	{
		this.dopplerFactor = { value: 1.0 };
		this.speedOfSound = { value: 343.3 };

		this.setPosition = function (x, y, z) { };
		this.setOrientation = function (x, y, z, xUp, yUp, zUp) { };
		this.setVelocity = function (x, y, z) { };
	}
	...
	function AudioContext()
	{
		...
		this.createDynamicsCompressor = function (owner)
		{
			return setOwner(new DynamicsCompressorNode(this), owner);
		};
